...
# inject single entity
def inject(medida, s = requests.Session()):
	if len(medida) < 4: return 200
	if type(medida) != list: medida = [medida]
	m_json = json.dumps(medida)
	try: res = s.post(scorpio_dir_upsert, headers=headers_post, data=m_json)
	except: logger.error("Request to Scorpio Broker failed"); return 400
	sc = res.status_code
	if(sc >= 400): logger.error("HTTP "+str(sc)+" when injecting "+medida[0]["id"])
	else: logger.info("Injected "+medida[0]["id"])
	return sc

# inject batch of entities (may be an array of 1 entity)
def injectBatch(medidas, s = requests.Session()):
	if len(medidas) == 0: return 200
	if type(medidas) != list: return 400
	m_json = json.dumps(medidas)
	try: res = s.post(scorpio_dir_upsert, headers=headers_post, data=m_json)
	except: logger.error("Request to Scorpio Broker failed"); return 400
	sc = res.status_code
	if(sc >= 400): logger.error("HTTP "+str(sc)+" when injecting batch, including "+medidas[0]["id"])
	else: logger.info("Injected batch, including "+medidas[0]["id"])
	return sc
